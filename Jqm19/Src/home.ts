...
namespace Home {
	...
	namespace Page1 {
		...
		$(document).on("click", "#location_menu > li > .ui-btn", function (event: Event) {
			try {
                $(this).blur();
		...
		$(document).on("click", "#subwindow_menu > li > .ui-btn", function (event: Event) {
            try {
                $(this).blur();
                let cmd = $(this).attr("cmd");
                switch (cmd) {
		...
		$(document).on("click", "#menu3 > li > .ui-btn", function (event: Event) {
			try {
                $(this).blur();
                let cmd = $(this).attr("cmd");
				switch (cmd) {
        ...
        $(document).on("click", "#popup_menu > li > .ui-btn", function (event: Event) {
            try {
                $(this).blur();
                let cmd = $(this).attr("cmd");
                switch (cmd) {
                    ...
                    case "msgbox":
                        {
                            cm.MsgBox.show("メッセージ・メッセージ・メッセージ<br>宜しいですか？", "確認", true)
								.onClosed = function (result: boolean) {
									if (result === true) {
										alert("OK");
									}
								};
	...
	namespace Page2 {
        ...
        $(document).on("click", "#page2 li > .ui-btn", function (event: Event) {
            try {
                ...
                switch (cmd) {
                    case "list":
                        {
                            let lst = $("#page2_popup1");
                            let ipt = $(this).closest("li").find("input");	// 対象Input

                            // リスト選択
                            lst.find("li > .ui-btn").off("click").on("click", function (event: Event) {
                                let txt = $(this).text();
                                if (txt) {
                                    ipt.val(txt);
                                    Popup.close();
                            ...
                            // リスト表示
                            let pos = $(this).offset();
                            let x = $(window).width() - pos.left - $(this).outerWidth();	// ボタン左端
                            let y = pos.top + $(this).outerHeight();						// ボタン下端
                            Popup.open(lst, {
                                x2: x
                                , y1: y
                                , restoreScrollPos: true
		...
		$(document).on("click", "#page2_select1", function (event: Event){
			try {
         ...
         * フォーカス
         */
        $(document).on("focusin", "input, button, textarea, .ui-btn", function (event: Event) {
